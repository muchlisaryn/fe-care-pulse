...
/**
 * Jenis kuitansi — sekaligus penentu anggota mana yang muncul di dropdown.
 *
 ...
 *
 * Nilainya datang dari HALAMAN yang memakai form ini (`/baru/kelompok` atau
 * `/baru/pribadi`) dan tetap sepanjang pengisian: `transaction_type` tersimpan
 * di header, jadi satu kuitansi memang hanya berjenis satu.
 */
export type Tipe = "kelompok" | "pribadi"
...
type Rincian = {
  ...
  rencana: Rencana
  /**
   * Ketua kelompok saat rincian ini dibuat.
   *
   * Ikut disimpan supaya tombol Ubah bisa mengembalikan saringan dropdown
   * anggotanya seperti semula. Tanpa itu, anggota yang sedang diubah tidak ada
   * di daftar pilihan yang sedang aktif dan tidak bisa diganti ke anggota lain.
   */
...
/**
 * Form pembuatan kuitansi iuran.
 *
 * Dipakai dua halaman — `/nafsul/transaksi/baru/kelompok` dan
 * `.../baru/pribadi` — yang masing-masing mematok satu `tipe`. Jenisnya tidak
 * lagi bisa berpindah di tengah pengisian: `transaction_type` tersimpan di
 * header, jadi satu kuitansi memang hanya berjenis satu, dan potongan/jasa
 * ketua kelompok hanya berlaku untuk setoran kelompok.
 */
export default function TransaksiForm({ tipe }: { tipe: Tipe }) {
  const t = useT()
   ...
   *
   * Jenis kuitansi tidak ikut direset: ia milik halamannya, bukan isian form.
   */
  function resetForm() {
    if (timer.current) clearTimeout(timer.current)
    permintaan.current++
   ...
   *
   * Ketuanya ikut dikembalikan seperti saat rincian itu dibuat, supaya dropdown
   * anggotanya berisi daftar yang sama.
   */
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        {/*
          Jenis kuitansi ada di JUDUL, bukan lagi sebagai tab yang bisa ditekan:
          halamannya sendiri yang menentukan, jadi tidak ada keadaan setengah
          jalan yang isinya bertentangan dengan jenisnya.
        */}
        <PageHeader
          className="mb-5"
          title={t("nafsulTransaksi.newTitle_" + tipe)}
          subtitle={t("nafsulTransaksi.newSubtitle_" + tipe)}
        />
      </div>
