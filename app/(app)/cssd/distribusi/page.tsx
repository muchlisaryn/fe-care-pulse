...
export default function DistribusiPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!active) return
        setFormError("distribution.errLoadBmhp")
      }
    })()
    return () => {
      active = false
  ...
  const bmhpOptions = bmhps.map((b) => ({
    value: String(b.id),
    label: t("distribution.stockLeft", { name: b.name, qty: b.stock_qty, unit: b.unit }),
  }))
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setFormError(firstError ?? res?.data?.message ?? t("distribution.errSave"))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[340px_1fr]">
        ...
        <Card className="p-0">
          <div className="border-b border-gray-100 px-4 py-3">
            <p className="mb-2 text-sm font-semibold text-gray-700">{t("distribution.listTitle")}</p>
          ...
          <div className="max-h-[640px] divide-y divide-gray-100 overflow-y-auto">
            {loading ? (
              ...
              <p className="py-10 text-center text-sm text-gray-400">{t("distribution.empty")}</p>
            ) : (
              visibleDistributions.map((d) => (
                ...
                >
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    {d.status === "dibatalkan" ? (
                      <Badge variant="danger">{t("distribution.canceled")}</Badge>
                    ) : (
                      <Badge variant="success">{t("distribution.distributed")}</Badge>
                    )}
                  </div>
                  <dl className="space-y-0.5 text-xs text-gray-600">
                    <div className="flex gap-1">
                      <dt className="w-16 shrink-0 text-gray-400">{t("distribution.sender")}</dt>
                      <dd>: {d.sender ?? "—"}</dd>
                    </div>
                    <div className="flex gap-1">
                      <dt className="w-16 shrink-0 text-gray-400">{t("distribution.unit")}</dt>
                      <dd>: {d.room?.name ?? "—"}</dd>
                    </div>
                    <div className="flex gap-1">
                      <dt className="w-16 shrink-0 text-gray-400">{t("distribution.recipient")}</dt>
                      <dd>: {d.receiver ?? "—"}</dd>
                    </div>
                    <div className="flex gap-1">
                      <dt className="w-16 shrink-0 text-gray-400">{t("common.date")}</dt>
                      <dd>: {formatDateTime(d.distributed_at, lang)}</dd>
                    </div>
                  </dl>
                </button>
              ))
        ...
        <Card>
          <h2 className="mb-4 text-base font-semibold text-gray-900">{t("distribution.formTitle")}</h2>

          <div className="space-y-5">
            {/* Unit */}
            <div className="space-y-1.5">
              <Label>
                {t("distribution.unit")} <span className="text-red-500">*</span>
              </Label>
              <SelectSearch
                options={roomOptions}
                value={roomId}
                onChange={setRoomId}
                placeholder={t("distribution.selectRoom")}
                searchPlaceholder={t("distribution.searchRoom")}
              />
            </div>
            ...
            <div className="rounded-lg border border-gray-200">
              <div className="border-b border-gray-100 px-4 py-2.5">
                <p className="text-sm font-semibold text-gray-700">{t("distribution.bmhpSelection")}</p>
              </div>
              ...
              <div className="space-y-3 px-4 py-3">
                <div className="flex flex-col gap-2 sm:flex-row">
                  <div className="flex-1">
                    <SelectSearch
                      ...
                      placeholder={t("distribution.selectBmhp")}
                      searchPlaceholder={t("distribution.searchBmhp")}
                    />
                  </div>
                  <Input
                    type="number"
                    ...
                    className="w-24"
                    placeholder={t("distribution.qtyPlaceholder")}
                  />
                  <Button
                    type="button"
                ...
                {formItems.length === 0 ? (
                  <div className="flex flex-col items-center gap-1 py-6 text-gray-400">
                    <p className="text-sm">{t("distribution.noBmhpPicked")}</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full min-w-[440px] text-sm">
                      <thead>
                        <tr className="border-b border-gray-100 bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-400">
                          ...
                          <th className="px-3 py-2 w-24">{t("common.quantity")}</th>
            ...
            <div className="rounded-lg border border-gray-200">
              <div className="border-b border-gray-100 px-4 py-2.5">
                <p className="text-sm font-semibold text-gray-700">{t("distribution.recipientInfo")}</p>
              </div>
              <div className="grid grid-cols-1 gap-4 px-4 py-3 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label>
                    {t("distribution.sender")} <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    value={sender}
                    onChange={(e) => setSender(e.target.value)}
                    placeholder={t("distribution.senderPlaceholder")}
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>{t("distribution.distributionDate")}</Label>
                  <Input
                    type="datetime-local"
                    value={distributedAt}
                ...
                <div className="space-y-1.5">
                  <Label>
                    {t("distribution.recipient")} <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    value={receiver}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder={t("common.note")} />
                </div>
              </div>
            </div>

            {formError && (
              <div className="rounded-lg bg-red-50 px-4 py-2.5 text-sm text-red-600">{t(formError)}</div>
            )}
      ...
      <Modal
        ...
        title={detail ? `${t("distribution.detailTitle")} — ${detail.code}` : t("distribution.detailTitle")}
        size="lg"
        footer={
        ...
        {detailLoading ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              <Info label={t("distribution.unit")} value={detail.room?.name} />
              <Info
                label={t("common.status")}
                value={
                  detail.status === "dibatalkan"
                    ? t("distribution.canceled")
                    : t("distribution.distributed")
                }
              />
              <Info label={t("distribution.sender")} value={detail.sender} />
              <Info label={t("distribution.recipient")} value={detail.receiver} />
              <Info label={t("common.date")} value={formatDateTime(detail.distributed_at, lang)} />
              <Info label={t("common.note")} value={detail.note} />
            </div>
            <div className="overflow-x-auto rounded-lg border border-gray-200">
